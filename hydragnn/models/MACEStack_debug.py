...
class MACEStack(Base):
    ...
    def _embedding(self, data):
        ...
        # Debug: detect zero-length edges (can cause NaNs in MACE radial basis)
        zero_mask = edge_dist.squeeze(-1) == 0
        if torch.any(zero_mask):
            zero_count = int(zero_mask.sum().item())
            sample_idx = torch.nonzero(zero_mask, as_tuple=False).flatten()[:10]
            sample_pairs = (
                data.edge_index[:, sample_idx].detach().cpu().tolist()
                if sample_idx.numel() > 0
                else []
            )
            print(
                f"[MACE] Found {zero_count} zero-length edges. Sample edge_index pairs: {sample_pairs}",
                flush=True,
            )
