...
class InteratomicPotentialMixin:
    ...
    def _compute_enhanced_geometric_features(self, data, conv_args):
        ...
        shifts = getattr(
            data,
            "edge_shifts",
            torch.zeros((edge_index.size(1), 3), device=edge_index.device),
        )
    ...
    def _compute_local_environment_features(
        self, positions, edge_index, edge_vectors, edge_lengths
    ):
        """
        ...
        coord_numbers = torch_scatter.scatter_add(
            torch.ones_like(edge_lengths.squeeze()), edge_index[1], dim_size=num_nodes
        )

        # Compute average distances to neighbors
        avg_distances = torch_scatter.scatter_mean(
            edge_lengths.squeeze(), edge_index[1], dim_size=num_nodes
        )

        # Handle nodes with no neighbors
        avg_distances = torch.where(
            coord_numbers > 0, avg_distances, torch.zeros_like(avg_distances)
        )

        # Compute local density (inverse of average distance)
        local_density = torch.where(
            avg_distances > 0,
            1.0 / (avg_distances + 1e-6),
            torch.zeros_like(avg_distances),
        )
    ...
    def _compute_three_body_interactions(self, node_features, data, conv_args):
        ...
        for idx, (center_atom, n_neighbors) in enumerate(zip(j_unique, j_counts)):
            if n_neighbors >= 2:
                ...
                center_edges = torch.arange(
                    edge_start_idx, edge_end_idx, device=edge_index.device
                )
                ...
                neighbor_features = node_features[
                    i[center_edges]
                ]  # Features of neighbor atoms
                ...
                if neighbor_features.size(0) > 1:
                    three_body_contrib = self.three_body_mlp(
                        torch.cat(
                            [
                                neighbor_features.mean(dim=0, keepdim=True).expand(
                                    1, -1
                                ),
    ...
    def forward(self, data):
        ...
        radius_graph = RadiusGraph(
            r=radius, loop=False, max_num_neighbors=max_neighbours
        )
        ...
        if self.use_three_body_interactions:
            inv_node_feat = self._compute_three_body_interactions(
                inv_node_feat, data, conv_args
            )

        if self.use_atomic_environment_descriptors:
            inv_node_feat = self._apply_atomic_environment_descriptors(
                inv_node_feat, conv_args
            )
...
class InteratomicPotentialBase(InteratomicPotentialMixin, Base):
    ...
    3. Energy-force loss calculation for MLIP training
