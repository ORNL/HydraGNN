...
def _get_absolute_mapping(name: str):
    ...
    # e.g., `hydragnn.utils.model.uma._vendored.fairchem.core.tasks.base_task.BaseTask`
    # we can use importlib to get the module (e.g., `hydragnn.utils.model.uma._vendored.fairchem.core.tasks.base_task`)
...
class Registry:
    ...
    def register_task(cls, name: str):
        ...
        Usage::
            from hydragnn.utils.model.uma._vendored.fairchem.core.common.registry import registry
            from hydragnn.utils.model.uma._vendored.fairchem.core.tasks import BaseTask
    ...
    def register_dataset(cls, name: str):
        ...
        Usage::
            ...
            from hydragnn.utils.model.uma._vendored.fairchem.core.common.registry import registry
            from hydragnn.utils.model.uma._vendored.fairchem.core.datasets import BaseDataset
    ...
    def register_model(cls, name: str):
        ...
        Usage::
            ...
            from hydragnn.utils.model.uma._vendored.fairchem.core.common.registry import registry
            from hydragnn.utils.model.uma._vendored.fairchem.core.modules.layers import CGCNNConv
    ...
    def register_logger(cls, name: str):
        ...
        def wrap(func: Callable[..., R]) -> Callable[..., R]:
            from hydragnn.utils.model.uma._vendored.fairchem.core.common.logger import Logger
    ...
    def __import_error(cls, name: str, mapping_name: str) -> RuntimeError:
        ...
        if existing_cls_path is not None:
            ...
            existing_cls_path = "hydragnn.utils.model.uma._vendored.fairchem.core.trainers.ForcesTrainer"
    ...
    def get_class(cls, name: str, mapping_name: str):
        ...
        # mapping be class path of type `{module_name}.{class_name}` (e.g., `hydragnn.utils.model.uma._vendored.fairchem.core.trainers.ForcesTrainer`)
        if name.count(".") < 1:
