...
def linear_mixing_internal_energy_data(
    path: str,
    ...
    #   NODAL_FEATURE = MODULUS( NODE_INDEX, NODE_FEATURE )

    ###############################################################################################
    ##########################   FORMULAS FOR GLOBAL AND NODAL OUTOUTS  ###########################
    ###############################################################################################

    #   GLOBAL_OUTPUT = SUM_OVER_NODES ( NODAL_OUTPUT1 )
    #   NODAL_OUTPUT1(X) = X
    ...
    for configuration in range(number_configurations):
        ...
        cluster_ids_x = torch.randint(0, 2, (number_nodes, 1))

        node_feature = cluster_ids_x

        updated_table = torch.cat(
            (node_feature, node_ids, positions, cluster_ids_x),
            1,
        )
        numpy_updated_table = updated_table.detach().numpy()

        total_value = torch.sum(cluster_ids_x)
