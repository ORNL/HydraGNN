...
def test_integration_final():
    ...
    try:
        ...
        print(
            f"     Backend: {'OpenEquivariance' if getattr(linear, 'using_openequivariance', False) else 'e3nn'}"
        )
        ...
        print(
            f"   ✓ OptimizedSphericalHarmonics: {vectors.shape} -> {sh_features.shape}"
        )
        print(
            f"     Backend: {'OpenEquivariance' if getattr(sh, 'using_openequivariance', False) else 'e3nn'}"
        )
        ...
        try:
            ...
            print(
                f"   ✓ OptimizedTensorProduct: {x1.shape} x {x2.shape} -> {y_tp.shape}"
            )
            print(
                f"     Backend: {'OpenEquivariance' if getattr(tp, 'using_openequivariance', False) else 'e3nn'}"
            )
    ...
    try:
        ...
        config_enabled = {
            "NeuralNetwork": {"Architecture": {"enable_openequivariance": True}}
        }

        enable_oe = config_enabled["NeuralNetwork"]["Architecture"][
    ...
    try:
        ...
        weights_block = TensorProductWeightsBlock(
            num_elements=2, num_edge_feats=3, num_feats_out=4
        )
    ...
    if success:
        ...
        print(
            "✓ Optional OpenEquivariance usage via 'enable_openequivariance' parameter"
        )
