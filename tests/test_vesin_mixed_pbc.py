...
def create_2d_slab(
    n_atoms: int = 20, vacuum: float = 15.0, seed: int = 42
) -> ase.Atoms:
    """
    ...
    positions[:, 2] = (
        positions[:, 2] * 2.0 + vacuum - 1.0
    )  # z in thin layer around center
...
def create_1d_chain(
    n_atoms: int = 15, vacuum: float = 15.0, seed: int = 42
) -> ase.Atoms:
    """
...
def compare_neighbor_lists(
    ...
    if len(common) > 0:
        ...
        def make_dist_dict(i, j, S, d):
            return {
                (int(ii), int(jj), tuple(ss)): dd for ii, jj, ss, dd in zip(i, j, S, d)
            }
...
def test_2d_slab_comparison():
    ...
    print(
        f"  Expanded cell diag = {np.diag(atoms_expanded.cell.array)[:2]}... (truncated)"
    )
    ...
    result = compare_neighbor_lists(
        i_ase, j_ase, d_ase, S_ase, i_vesin, j_vesin, d_vesin, S_vesin
    )
...
def test_various_mixed_pbc():
    ...
    for pbc in pbc_configs:
        ...
        # Vesin with workaround
        atoms_expanded = expand_cell_for_vesin(atoms, cutoff=cutoff)
        i_vesin, j_vesin, d_vesin, S_vesin = get_vesin_neighbor_list(
            atoms_expanded, cutoff
        )
        ...
        print(
            f"  {status}: ASE={result['n_edges_1']} edges, vesin={result['n_edges_2']} edges, "
...
def test_vesin_direct_mixed_pbc():
    ...
    for pbc in pbc_configs:
        ...
        try:
            ...
            i_vesin, j_vesin, S_vesin = vesin.ase_neighbor_list(
                "ijS", a=atoms, cutoff=cutoff
            )
            pos = atoms.positions
            cell = atoms.cell.array
            ...
            i_full, j_full, S_full = vesin.ase_neighbor_list(
                "ijS", a=atoms_full_pbc, cutoff=cutoff
            )
            ...
            if len(i_vesin) == len(i_full):
                ...
                full_result = compare_neighbor_lists(
                    i_vesin, j_vesin, d_vesin, S_vesin, i_full, j_full, d_full, S_full
                )
                if full_result["edges_match"]:
                    print(
                        f"  ⚠ Behavior IDENTICAL to pbc=[True,True,True] - vesin may ignore mixed PBC"
                    )
...
def test_edge_cases():
    ...
    all_passed = all_passed and passed
    print(
        f"  {'✓ PASS' if passed else '✗ FAIL'}: ASE={result['n_edges_1']}, vesin={result['n_edges_2']}"
    )
...
if __name__ == "__main__":
    ...
    if all_passed:
        print(
            "All tests passed! The large-cell workaround correctly emulates mixed PBC."
        )
    else:
