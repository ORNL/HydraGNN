...
@pytest.mark.parametrize(
    "species",
    [
        {"name": "elements", "level": "node", "dim": 1, "role": "species"},
        {"name": "atomic_numbers", "level": "graph", "dim": 1, "role": "species"},
        {"name": "atomic_numbers", "level": "node", "dim": 2, "role": "species"},
    ],
)
def test_species_role_requires_canonical_atomic_number_contract(species):
    with pytest.raises(
        ValueError, match="must have name 'atomic_numbers', level 'node', and dim 1"
    ):
        parse_variable_schema({"inputs": [species], "outputs": []})
