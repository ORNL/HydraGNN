...
@pytest.mark.skipif(not openequivariance_available, reason="OpenEquivariance not available")
def pytest_linear_equivalence():
    ...
    for irreps_in, irreps_out in test_cases:
        ...
        linear_oe = OptimizedLinear(irreps_in, irreps_out, internal_weights=True, shared_weights=True)
        
        # Force e3nn backend for comparison
        original_state = force_e3nn_backend()
        linear_e3nn = OptimizedLinear(irreps_in, irreps_out, internal_weights=True, shared_weights=True)
        restore_backend_state(original_state)
        
        # Copy weights from e3nn to OpenEquivariance for fair comparison
        if hasattr(linear_oe, 'using_openequivariance') and linear_oe.using_openequivariance:
            # If OpenEquivariance is actually being used, copy weights
            if hasattr(linear_e3nn.linear, 'weight') and hasattr(linear_oe.linear, 'weight'):
                with torch.no_grad():
        ...
        assert torch.allclose(y_oe, y_e3nn, rtol=RTOL, atol=ATOL), \
...
def pytest_spherical_harmonics_equivalence():
    ...
    for l_max in l_max_values:
        ...
        sh_oe = OptimizedSphericalHarmonics(irreps_sh, normalize=True, normalization="component")
        
        # Force e3nn backend
        original_state = force_e3nn_backend()
        sh_e3nn = OptimizedSphericalHarmonics(irreps_sh, normalize=True, normalization="component")
        restore_backend_state(original_state)
...
def pytest_tensor_product_equivalence():
    ...
    for irreps_in1, irreps_in2, irreps_out in test_cases:
        print(f"Testing OptimizedTensorProduct: {irreps_in1} ⊗ {irreps_in2} -> {irreps_out}")
        ...
        if hasattr(tp_oe, 'using_openequivariance') and tp_oe.using_openequivariance:
            if hasattr(tp_e3nn.tensor_product, 'weight') and hasattr(tp_oe.tensor_product, 'weight'):
                with torch.no_grad():
...
def pytest_einsum_equivalence():
    ...
    for equation, shapes in test_cases:
        ...
        print(f"  ✓ Passed: max_diff={torch.max(torch.abs(result_optimized - result_torch)):.2e}")
...
def pytest_mace_block_equivalence():
    ...
    weights_block = TensorProductWeightsBlock(num_elements, num_edge_feats, num_feats_out)
    ...
    # Check numerical equivalence
    assert torch.allclose(result_optimized, result_standard, rtol=RTOL, atol=ATOL), \
        f"TensorProductWeightsBlock outputs differ: max_diff={torch.max(torch.abs(result_optimized - result_standard))}"
    
    print(f"  ✓ Passed: max_diff={torch.max(torch.abs(result_optimized - result_standard)):.2e}")
...
def pytest_precision_consistency():
    ...
    for dtype in dtypes:
        ...
        assert torch.allclose(result_optimized, result_torch, rtol=tolerance, atol=tolerance), \
            f"Precision inconsistency for {dtype}"
        
        print(f"    ✓ {dtype}: max_diff={torch.max(torch.abs(result_optimized - result_torch)):.2e}")
