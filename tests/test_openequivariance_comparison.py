...
from equivariance_compat import get_backend_info, is_openequivariance_available
...
def pytest_direct_e3nn_comparison():
    ...
    try:
        ...
        assert (
            result_pure.shape == result_compat.shape
        ), f"Shape mismatch: {result_pure.shape} vs {result_compat.shape}"

        # Verify both results are finite and have reasonable magnitudes
        assert torch.isfinite(
            result_pure
        ).all(), "Pure e3nn result contains non-finite values"
        assert torch.isfinite(
            result_compat
        ).all(), "Compatibility wrapper result contains non-finite values"
...
def pytest_openequivariance_comparison():
    ...
    if is_openequivariance_available():
        ...
        try:
            ...
            assert (
                result_e3nn.shape == result_oeq.shape
            ), f"Shape mismatch: {result_e3nn.shape} vs {result_oeq.shape}"

            # Verify both results are finite and have reasonable magnitudes
            assert torch.isfinite(
                result_e3nn
            ).all(), "e3nn result contains non-finite values"
            assert torch.isfinite(
                result_oeq
            ).all(), "OpenEquivariance result contains non-finite values"
        ...
        print(
            "  Install OpenEquivariance for full comparison: pip install openequivariance"
        )
