...
try:
    ...
    from hydragnn.utils.model.mace_utils.tools.torch_geometric.dataloader import DataLoader
    ...
    pytest.skip(f"Required HydraGNN modules not available: {e}", allow_module_level=True)
...
def create_mace_model(r_max=5.0, num_bessel=8, num_polynomial_cutoff=6, 
                     max_ell=2, interaction_cls_first="RealAgnosticResidualInteractionBlock",
                     num_interactions=2, hidden_irreps="32x0e + 32x1e + 16x2e",
                     atomic_energies=None, avg_num_neighbors=None):
...
def pytest_mace_e3nn_vs_openequivariance_training():
    ...
    try:
        ...
        print(f"Created {len(data_list)} samples with {data_list[0].atomic_numbers.shape[0]} atoms each")
        ...
        # Test both backends
        for backend_name, use_openequivariance in [("e3nn", False), ("OpenEquivariance", True)]:
            ...
            try:
                ...
                if use_openequivariance and not backend_available:
                    print(f"WARNING: OpenEquivariance not available, falling back to e3nn")
                ...
                for epoch in range(training_epochs):
                    try:
                        loss = train_model_epoch(model, dataloader, optimizer, loss_fn, device)
                ...
                with torch.no_grad():
                    ...
                    assert output.energy.shape[0] == test_batch.ptr.shape[0] - 1, "Energy shape mismatch"
                    assert output.forces.shape == test_batch.forces.shape, "Forces shape mismatch"
        ...
        if success_count >= 2:
            backend_names = [name for name, result in results.items() if result.get('success', False)]
            ...
            print(f"  Model parameters match: {params_match} ({params[0]:,} parameters)")
            ...
            successful_backend = next(name for name, result in results.items() if result.get('success', False))
            print(f"Only {successful_backend} succeeded - this may indicate backend availability issues")
        ...
        for backend_name, result in results.items():
            if result.get('success', False):
                print(f"✓ {backend_name}: Training successful (final loss: {result['final_loss']:.6f})")
            else:
                print(f"✗ {backend_name}: Training failed - {result.get('error', 'Unknown error')}")
