...
def test_openequivariance_basic():
    ...
    try:
        ...
        print(
            f"   Using OpenEquivariance: {getattr(linear, 'using_openequivariance', 'unknown')}"
        )
    ...
    try:
        ...
        print(
            f"   ✓ OptimizedSphericalHarmonics created: {type(sh.spherical_harmonics).__name__}"
        )
        print(
            f"   Using OpenEquivariance: {getattr(sh, 'using_openequivariance', 'unknown')}"
        )
    ...
    try:
        ...
        print(
            f"   ✓ optimized_einsum works correctly: {a.shape} x {b.shape} -> {c1.shape}"
        )
    ...
    try:
        ...
        enable_oe = updated_config["NeuralNetwork"]["Architecture"].get(
