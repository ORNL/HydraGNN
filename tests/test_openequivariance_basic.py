...
def pytest_mace_integration_basic():
    ...
    try:
        from hydragnn.utils.model.mace_utils.modules.blocks import TensorProductWeightsBlock
    except ImportError:
...
def pytest_configuration_parameter_handling():
    ...
    if "enable_openequivariance" not in config_no_param["NeuralNetwork"]["Architecture"]:
        config_no_param["NeuralNetwork"]["Architecture"]["enable_openequivariance"] = False
    
    assert config_no_param["NeuralNetwork"]["Architecture"]["enable_openequivariance"] is False
