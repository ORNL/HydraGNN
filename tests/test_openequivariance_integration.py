...
def test_openequivariance_integration():
    ...
    config_file = os.path.join(
        os.path.dirname(__file__), "LJ_openequivariance_test.json"
    )
    ...
    enable_oe = config["NeuralNetwork"]["Architecture"].get(
        "enable_openequivariance", False
    )
    ...
    try:
        ...
        final_enable_oe = updated_config["NeuralNetwork"]["Architecture"].get(
            "enable_openequivariance", False
        )
    ...
    try:
        with warnings.catch_warnings(record=True) as w:
            ...
            model = create_model_config(
                config=updated_config["NeuralNetwork"], verbosity=1
            )
            ...
            oe_warnings = [
                warning
                for warning in w
                if "openequivariance" in str(warning.message).lower()
            ]
            if oe_warnings:
            ...
            if hasattr(model, "using_openequivariance"):
                print(
                    f"   Model using OpenEquivariance: {model.using_openequivariance}"
                )
    ...
    try:
        ...
        print(
            f"   ✓ Forward pass successful, output shape: {[o.shape for o in output]}"
        )
