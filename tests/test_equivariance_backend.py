...
class PytestEquivarianceBackend:
    ...
    def pytest_backend_selection(self, reset_backend_state):
        ...
        config = {"NeuralNetwork": {"Architecture": {"equivariance_backend": "e3nn"}}}
    ...
    def pytest_instruction_normalization(self):
        ...
        expected = [(0, 1, 2, "uvw", True, 1.0), (1, 0, 3, "uvu", False, 1.0)]
    ...
    @pytest.mark.skipif(not E3NN_AVAILABLE, reason="e3nn not available")
    def pytest_e3nn_tensor_product_basic(
        self, reset_backend_state, sample_irreps, sample_data
    ):
    ...
    @pytest.mark.skipif(
        not (E3NN_AVAILABLE and OPENEQUIV_AVAILABLE),
    ...
    def pytest_backend_numerical_equivalence(self, reset_backend_state, sample_irreps):
        ...
        assert (
            max_diff < TOLERANCE
        ), f"Outputs differ by {max_diff:.2e}, exceeds tolerance {TOLERANCE:.2e}"
        assert (
            rel_error < TOLERANCE
    ...
    def pytest_shared_weights_equivalence(
        self, reset_backend_state, sample_irreps, shared_weights
    ):
        ...
        assert (
            max_diff < TOLERANCE
        ), f"shared_weights={shared_weights}: difference {max_diff:.2e} > {TOLERANCE:.2e}"
    ...
    def pytest_equivariance_property(self, reset_backend_state, sample_irreps):
        ...
        assert (
            max_diff < TOLERANCE
        ), f"Equivariance violated: difference {max_diff:.2e} > {TOLERANCE:.2e}"
    ...
    def pytest_gradient_equivalence(self, reset_backend_state, sample_irreps):
        ...
        # Forward passes
        output_e3nn = tp_e3nn(
            x1.clone().detach().requires_grad_(True),
        ...
        ), f"Output difference {output_diff:.2e} > {TOLERANCE:.2e}"
