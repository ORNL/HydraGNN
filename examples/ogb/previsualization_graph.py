...
import copy
##################################################################################################################
def smiles2graphlist(smileset, valueset, indexlists, set_flag='train', filedir='dataset/processed'):
...
if __name__ == '__main__':
   var_names = ["GAP"]
   datafile_processed = os.path.join(os.path.dirname(__file__), "dataset/pcqm4m_gap.pt")
   ...
   p.starmap(smiles2graphlist, zip([smileset]*num_proc,[valueset]*num_proc,ind_lists))
   p.close()
   p.join()
