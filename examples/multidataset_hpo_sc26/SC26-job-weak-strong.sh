...
export BATCH_SIZE=$((EFFECTIVE_BATCH_SIZE/NUM_NODES/8))
export NUM_EPOCH=4
...
for SCALE in `seq 0 $DEPTH`; do
    NNODES=$((NUM_NODES / 2**SCALE))
    export BATCH_SIZE=200

    cmd srun --nodelist="$JOB_NODELIST" -N$NNODES -n$((NNODES*8)) -c7 --gpus-per-task=1 --gpu-bind=closest \
    python -u $HYDRAGNN_ROOT/examples/multidataset_hpo_sc26/gfm_mlip_all_mpnn.py \
        ...
        --multi --ddstore --ddstore_width=$((32*8)) --multi_model_list=$MULTI_MODEL_LIST --batch_size=$BATCH_SIZE --num_epoch=$NUM_EPOCH \
...
for SCALE in `seq 0 $DEPTH`; do
    NNODES=$((NUM_NODES / 2**SCALE))
    export BATCH_SIZE=$((EFFECTIVE_BATCH_SIZE / NNODES / 8))
