...
MPNN_TYPE = "DimeNet"
WRAPPED_SCRIPT = os.path.join(os.path.dirname(__file__), "gfm_mlip_dimenet.py")
...
def run(trial, dequed=None):
    ...
    command = " ".join(
        [
            ...
            f"--basis_emb_size={trial.parameters['basis_emb_size']}",
            f"--envelope_exponent={trial.parameters['envelope_exponent']}",
            f"--int_emb_size={trial.parameters['int_emb_size']}",
            f"--out_emb_size={trial.parameters['out_emb_size']}",
            f"--num_after_skip={trial.parameters['num_after_skip']}",
            f"--num_before_skip={trial.parameters['num_before_skip']}",
            f"--num_radial={trial.parameters['num_radial']}",
            f"--num_spherical={trial.parameters['num_spherical']}",
            "--inputfile=gfm_mlip.json",
            "--multi",
            "--ddstore",
            '--multi_model_list="Alexandria,ANI1x,MPTrj,OC2020,OC2022,OC25,ODAC23,OMat24,OMol25,OMol25-neutral,OMol25-non-neutral,OC2025,OPoly2026,Nabla2DFT,QCML,QM7-X,transition1x"',
            "--num_epoch=10",
            f"--log={log_name}",
...
if __name__ == "__main__":
    ...
    problem.add_hyperparameter((8, 200), "basis_emb_size")
    problem.add_hyperparameter((4, 6), "envelope_exponent")
    problem.add_hyperparameter((16, 500), "int_emb_size")
    problem.add_hyperparameter((24, 4000), "out_emb_size")
    problem.add_hyperparameter((0, 2), "num_after_skip")
    problem.add_hyperparameter((0, 2), "num_before_skip")
    problem.add_hyperparameter((3, 12), "num_radial")
    problem.add_hyperparameter((2, 8), "num_spherical")
