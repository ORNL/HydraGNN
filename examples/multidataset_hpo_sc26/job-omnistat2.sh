...
export PYTHONPATH=/lustre/orion/world-shared/lrn070/jyc/frontier/HydraGNN:$PYTHONPATH
...
python -u $HYDRAGNN_ROOT/examples/multibranch_hpo_sc26/gfm_mlip_all_mpnn.py \
    --log=multibranch_hpo-$SLURM_JOB_ID-NN$SLURM_JOB_NUM_NODES-FSDP$HYDRAGNN_USE_FSDP --everyone \
    --inputfile=gfm_mlip.json --num_samples=$((BATCH_SIZE*HYDRAGNN_MAX_NUM_BATCH*NUM_EPOCH)) \
    --multi --ddstore --multi_model_list=$datadir7,$datadir8 --batch_size=$BATCH_SIZE --num_epoch=$NUM_EPOCH \
