...
## Torch Dataset for Clintox CSV format
class ClintoxDatasetFactory:
    def __init__(
        ...
        (
            ...
            ymean_feature,
            ystd_feature,
        ) = clintox_dataset_load(datafile, sampling=sampling, seed=seed)
...
class ClintoxDataset(torch.utils.data.Dataset):
...
if __name__ == "__main__":
    ...
    if args.preonly:
        ...
        (
            ...
            ymean_feature,
            ystd_feature,
        ) = clintox_dataset_load(datafile, sampling=args.sampling, seed=43)
    ...
    if args.format == "adios":
        ...
        fname = os.path.join(os.path.dirname(__file__), "dataset", "clintox.csv")
        fact = ClintoxDatasetFactory(fname, args.sampling, var_config=var_config)
        trainset = ClintoxDataset(fact, "trainset")
        valset = ClintoxDataset(fact, "valset")
        testset = ClintoxDataset(fact, "testset")
