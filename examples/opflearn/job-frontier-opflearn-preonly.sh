#!/bin/bash
#SBATCH -A LRN087
#SBATCH -J opflearn_pre
#SBATCH -o opflearn-preonly-%j.out
#SBATCH -e opflearn-preonly-%j.out
#SBATCH -t 01:00:00
#SBATCH -p batch
#SBATCH -q debug
#SBATCH -N 1
...
CASE_NAME=${CASE_NAME:-case14_ieee}
FORMULATION=${FORMULATION:-ACOPF}
MODELNAME=${MODELNAME:-OPFLearn_Solution}
TRAIN_SCRIPT=${TRAIN_SCRIPT:-train_opflearn_solution_homogeneous.py}
...
srun -N ${NHOSTS} \
    ...
    python -u "${TRAIN_SCRIPT}" --preonly \
    --case_name "${CASE_NAME}" --formulation "${FORMULATION}" \
    --modelname "${MODELNAME}" ${MAX_SAMPLES_ARG}
