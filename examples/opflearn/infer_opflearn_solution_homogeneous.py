"""Inference/evaluation for OPFLearn solution prediction (homogeneous).

Loads a trained OPFLearn_Solution model, runs it on the held-out test split,
and writes parity plots plus per-head MAE / test-error metrics. OPFLearn samples
are single-node tabular graphs, so only the homogeneous model applies.

Typical usage:
    python infer_opflearn_solution_homogeneous.py --modelname OPFLearn_Solution
"""
...
def _plot_parity(true_values, pred_values, out_dir, name="opf_solution"):
