...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "input_path", type=Path, help="Path to processed Parquet or raw CSV file."
    )
...
def main() -> int:
    ...
    input_path = (
        ...
        else (script_dir / args.input_path).resolve()
    )
    ...
    try:
        ...
        if args.json:
            ...
            logger.info(
                "Detected input columns: %d", len(report["detected_input_columns"])
            )
            logger.info(
                "Detected primal-output columns: %d",
                len(report["detected_primal_output_columns"]),
            )
            logger.info(
                "Detected dual-output columns: %d",
                len(report["detected_dual_output_columns"]),
            )
            logger.info("Raw angle range (rad): %s", report["raw_angle_range_rad"])
            logger.info(
                "Corrected angle range (rad): %s", report["corrected_angle_range_rad"]
            )
            logger.info(
                "Voltage magnitude range: %s", report["voltage_magnitude_range"]
            )
