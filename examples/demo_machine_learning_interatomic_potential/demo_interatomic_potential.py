...
def create_demo_molecular_data():
    ...
    # Atomic numbers
    atomic_numbers = torch.tensor([8, 1, 1], dtype=torch.float32).unsqueeze(
        1
    )  # O, H, H

    # Edge connectivity (all atoms connected)
    edge_index = torch.tensor(
        [[0, 0, 1, 1, 2, 2], [1, 2, 0, 2, 0, 1]],  # Source atoms  # Target atoms
        dtype=torch.long,
    )
    ...
    try:
        ...
        class DemoModel(InteratomicPotentialMixin):
            def __init__(self):
                ...
                self.num_branches = 1
                self.graph_shared = {
                    "branch-0": torch.nn.Linear(self.hidden_dim, self.hidden_dim)
                }
                self.var_output = 0
                self.config_heads = {
                    "node": [{"type": "branch-0", "architecture": {"type": "mlp"}}]
                }
        ...
        print(
            f"✓ Atomic environment descriptors: {model.use_atomic_environment_descriptors}"
        )
        ...
        print(
            f"  Features enhanced: {not torch.allclose(node_features, enhanced_features)}"
        )
