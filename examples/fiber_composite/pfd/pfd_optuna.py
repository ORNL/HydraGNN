...
def objective(trial):
    ...
    model_type = trial.suggest_categorical("model_type", ["PNA"])
    hidden_dim = trial.suggest_int("hidden_dim", 8, 256)
    num_conv_layers = trial.suggest_int("num_conv_layers", 1, 5)
    num_headlayers = trial.suggest_int("num_headlayers", 1, 5)
    dim_headlayers = [
        trial.suggest_int(f"dim_headlayer_{i}", 8, 256) for i in range(num_headlayers)
    ]
    num_sharedlayers = trial.suggest_int("num_sharedlayers", 1, 5)
    dim_sharedlayers = trial.suggest_int("dim_sharedlayers", 8, 256)
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode="min", factor=0.5, patience=5, min_lr=0.000005
    )
...
if __name__ == "__main__":
    ...
    filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pf_optuna.json")
