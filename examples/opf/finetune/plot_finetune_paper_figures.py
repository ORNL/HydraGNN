...
# ── output directory ─────────────────────────────────────────────────────────
LATEX_DIR = (
    "/lustre/orion/lrn078/proj-shared/hydragnn_opf_tsg_project_with_layer_references"
)
...
def _parse_ft1_dirname(name):
    ...
    m = re.fullmatch(
        ...
        r"(full|partial|head_only|full_scratch)_n(\d+)",
        name,
    )
    if not m:
        return None
...
def plot_ft1_sample_efficiency(df):
    ...
    for c_idx, (_, label, _, _) in enumerate(metrics):
        ...
        axes[1, c_idx].set_title(f"{ARCH_LABELS[ARCH_ORDER[1]]}", fontweight="bold")
