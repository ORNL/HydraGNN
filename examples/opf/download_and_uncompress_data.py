...
def _parallel_download_and_extract_opf(
    ...
    assigned_groups = list(range(rank, num_groups, world_size))
    logging.info(
    ...
    for group_idx in assigned_groups:
        ...
        if not os.path.isfile(archive_path):
            ...
            logging.info(
                "OPF parallel fetch: rank=%d case=%s group=%d action=skip_download",
                rank,
                case_name,
                group_idx,
            )
        ...
        if os.path.isdir(group_dir):
            logging.info(
                "OPF parallel extract: rank=%d case=%s group=%d action=skip_extract",
                rank,
                case_name,
                group_idx,
            )
