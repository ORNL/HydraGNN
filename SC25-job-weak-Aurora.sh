...
[ -z $BATCH_SIZE ] && BATCH_SIZE=106 ## 320 (Perlmutter) 160 (Frontier) 106 (Aurora)
export BATCH_SIZE=$BATCH_SIZE


mpiexec -n ${NTOTPROCS} --ppn ${NPROCS_PER_NODE} --depth=${NDEPTH} --cpu-bind depth \
python -u ./examples/multibranch/train.py --log=GFM_taskparallel_weak-$PBS_JOBID-NN$NNODES-BS$BATCH_SIZE-TP0-DD$HYDRAGNN_DDSTORE_METHOD-NW$HYDRAGNN_NUM_WORKERS --everyone \
