...
def test_equivariance():
    ...
    for axis in ['x', 'y', 'z']:
        for angle in [np.pi/6, np.pi/4, np.pi/3, np.pi/2]:
            ...
            for i in range(3):
                for j in range(3):
                    vector_out_orig_rotated[:, i, :] += R[i, j] * vector_out_orig[:, j, :]
            
            vector_diff = torch.norm(vector_out_orig_rotated - vector_out_rot)
            vector_equivariant = vector_diff < 1e-3  # Slightly more tolerance for vectors
            ...
            print(f"  Vector equivariant: {vector_equivariant} (diff: {vector_diff:.6f})")
